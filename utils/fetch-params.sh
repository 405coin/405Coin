...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/405CoinParams"
else
    PARAMS_DIR="$HOME/.405Coin-params"
fi
...
DOWNLOAD_URL="https://github.com/405coin/405сoin/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with 405_DISABLE_SOMETHING=1
405_DISABLE_WGET="${405_DISABLE_WGET:-}"
#405_DISABLE_IPFS="${405_DISABLE_IPFS:-}"
405_DISABLE_CURL="${405_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$405_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$405_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$405_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the 405Coin parameters!
...
This script will fetch the 405Coin SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common 405Coin SNARK parameters. Note that it is
