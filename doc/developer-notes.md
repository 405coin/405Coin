...
- [Development guidelines](#development-guidelines)
    - [General 405Coin Core](#general-405Coin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
The 405Coin uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
405Coin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/405coin/405сoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_405Coin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_raptoreu --log_level=test_suite
$ valgrind -v --leak-check=full src/405Coind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_405Coin.coverage/index.html`.
```

**Sanitizers**

405Coin Core can be compiled with various "sanitizers" enabled, which add
...
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_405Coin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_405Coin --log_level=test_suite
$ valgrind -v --leak-check=full src/405Coind -printtoconsole
```
...
However, in open source software such as 405Coin Core, where everyone uses
...
pay attention to for reviewers of 405Coin Core code.

General 405Coin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: 405Coin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against 405Coin Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `405Coin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `405Coin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of 405Coin Core, the wallet was always
