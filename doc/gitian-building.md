...
*Setup instructions for a Gitian build of 405Coin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the 405Coin
...
- [Building 405Coin Core](#building-405Coin-core)
...
Clone the git repositories for 405Coin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/raptor3um/405Coin
...
in the 405Coin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building 405Coin Core
----------------

To build 405Coin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the 405Coin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/405Coin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/raptor3um/405Coin
...
For example:
```bash
URL=https://github.com/crowning-/405Coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit 405Coin=${COMMIT} --url 405Coin=${URL} ../405Coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit 405Coin=${COMMIT} --url 405Coin=${URL} ../405Coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit 405Coin=${COMMIT} --url 405Coin=${URL} ../405Coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the 405Coin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../405Coin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/405Coin
SIGPATH=/some/root/path/405Coin-detached-sigs

./bin/gbuild --url 405Coin=${BTCPATH},signature=${SIGPATH} ../405Coin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/405Coin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/405Coin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/405Coin-osx-build.assert
```
...
[405Coin/gitian.sigs](https://github.com/raptor3um/gitian.sigs/) repository, or if that's not possible to create a pull
request.
