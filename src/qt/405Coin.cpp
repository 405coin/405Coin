...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase,
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in 405Coin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in 405Coin.qrc)
...
static const char *qt_argv = "405-qt";
...
void BitcoinApplication::createSplashScreen(const NetworkStyle *networkStyle) {
    splashScreen = new SplashScreen(m_node, nullptr, networkStyle);
    splashScreen->show();
    connect(this, &BitcoinApplication::requestedInitialize, splashScreen, &SplashScreen::handleLoadWallet);
    connect(this, &BitcoinApplication::splashFinished, splashScreen, &SplashScreen::finish);
    connect(this, &BitcoinApplication::requestedShutdown, splashScreen, &QWidget::close);
    connect(splashScreen, &QObject::destroyed, this, [this]() {
        splashScreen = nullptr;
    });
}
...
void BitcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info) {
    ...
    if (success) {
        ...
        if (splashScreen) {
            connect(splashScreen, &SplashScreen::readyToLaunch, this, &BitcoinApplication::handleSplashReady, Qt::UniqueConnection);
...
void BitcoinApplication::handleSplashReady() {
    showMainWindow();
}

void BitcoinApplication::showMainWindow() {
    if (mainWindowShown || !window || !clientModel) {
        return;
    }
    ...
    if (!gArgs.GetBoolArg("-min", false)) {
        ...
        // Tray icon handles visibility when minimizing to tray.
    } else {
...
void BitcoinApplication::handleRunawayException(const QString &message) {
    QMessageBox::critical(nullptr, "Runaway exception", BitcoinGUI::tr(
            "A fatal error occurred. 405Coin Core can no longer continue safely and will quit.") + QString("\n\n") +
                                                        message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs() {
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"405Coin Core - \""),
                 ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
...
int GuiMain(int argc, char *argv[]) {
    ...
    Q_INIT_RESOURCE(405Coin);
    Q_INIT_RESOURCE(405Coin_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse 405Coin.conf
    ...
    // 405Coin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
