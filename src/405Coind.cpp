...
static bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/405Coin.conf are parsed in qt/405Coin.cpp's main()
    SetupServerArgs();
    std::string error;
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(strprintf(
                        "Error: Command line contains unexpected token '%s', see 405Coind -h for a list of options.\n",
                        argv[i]));
            }
        }

        // -server defaults to true for 405Coind but not for the GUI so do this here
...
int main(int argc, char *argv[]) {
    ...
    // Connect 405Coind signal handlers
    noui_connect();
