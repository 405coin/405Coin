...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // users ignore all instructions on 405Coincentral etc. and do not save them...
...
void gobject_vote_conf_help(const JSONRPCRequest &request) {
    RPCHelpMan{"gobject vote-conf",
               "Vote on a governance object by smartnode configured in 405Coin.conf\n",
               {
...
static UniValue gobject_vote_conf(const JSONRPCRequest &request) {
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("405Coin.conf", statusObj);
...
[[noreturn]] void gobject_help() {
    RPCHelpMan{"gobject",
        ...
        "  list-prepared      - Returns a list of governance objects prepared by this wallet with \"gobject prepare\"\n"
               ...
               #endif // ENABLE_WALLET
               "  vote-conf          - Vote on a governance object by smartnode configured in 405Coin.conf\n"
#ifdef ENABLE_WALLET
...
static const CRPCCommand commands[] =
        { //  category              name                      actor (function)         argNames
                //  --------------------- ------------------------  -----------------------  ----------
                /* 405Coin features */
                {"405Coin", "getgovernanceinfo",   &getgovernanceinfo,   {}},
                {"405Coin", "getsuperblockbudget", &getsuperblockbudget, {"index"}},
                {"405Coin", "gobject",             &gobject,             {}},
                {"405Coin", "voteraw",             &voteraw,             {"tx_hash", "tx_index", "gov_hash", "signal", "outcome", "time", "sig"}},
